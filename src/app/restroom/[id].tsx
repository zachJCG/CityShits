...
export default function RestroomDetailScreen() {
  ...
  const loadData = useCallback(async () => {
    if (!id) return;
    const r = await getRestroomById(db, id);
    setRestroom(r);
    const revs = await getReviewsForRestroom(db, id);
    setReviews(revs);
  }, [db, id]);
  ...
  const handleSubmitReview = async () => {
    ...
    await updateRestroomAverages(db, id);
    ...
    await loadData();
    Alert.alert('Review Dropped! 💩', 'Thanks for contributing to the throne database.');
  };
      ...
      <View style={styles.section}>
        ...
        {showReviewForm && (
          <View style={styles.reviewForm}>
            ...
            <RatingStars
              label="🔇 Soundproofing"
              value={newSoundproofing}
              onChange={setNewSoundproofing}
            />
            <TextInput
